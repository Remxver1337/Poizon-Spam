...
# Максимальная длина текста для кнопки-ссылки
MAX_URL_TEXT_LENGTH = 100
...
class SpamBot:
    ...
    async def handle_messages(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == "messages_create":
            ...
            create_text = (
                ...
                f"⚠️ *Максимальная длина сообщения: {MAX_URL_TEXT_LENGTH} символов*"
            )
    ...
    def create_spam_url(self, username: str, text: str) -> str:
        """Создание URL для спам-сообщения с проверкой длины"""
        if len(text) > MAX_URL_TEXT_LENGTH:
            return None
        ...
        try:
            ...
            logger.error(f"Ошибка при создании URL: {e}")
            return None
    ...
    async def show_users_for_spam(self, update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int, page: int = 0):
        ...
        for user_id_db, username in users:
            ...
            if variation_text:
                spam_url = self.create_spam_url(username, variation_text)
                
                if spam_url:
                    # Проверяем длину URL (Telegram имеет ограничения)
                    if len(spam_url) < 2000:  # Безопасный лимит для Telegram
                        ...
                        keyboard.append([InlineKeyboardButton(
                            f"❌ {username} (текст слишком длинный)", 
                            callback_data="no_action"
                        )])
                else:
                    keyboard.append([InlineKeyboardButton(
                        f"❌ {username} (ошибка создания ссылки)", 
                        callback_data="no_action"
                    )])
            else:
                keyboard.append([InlineKeyboardButton(
        ...
        # Если нет валидных пользователей для рассылки
        if valid_users_count == 0:
            warning_text = (
                f"⚠️ *Нет доступных пользователей для рассылки*\n\n"
                f"👥 *Чат: {chat_name}*\n"
                f"📄 *Страница: {page + 1}*\n\n"
                f"❌ *Причины:*\n"
                f"• Нет доступных вариаций сообщений\n"
                f"• Текст сообщений слишком длинный\n"
                f"• Ошибки при создании ссылок\n\n"
                f"💡 *Проверьте созданные сообщения и их длину*"
            )
        ...
        if target_user:
            ...
            if variation_text:
                ...
                if spam_url:
                    ...
                    await query.answer("❌ Ошибка: не удалось создать ссылку для сообщения!")
            else:
        ...
        if state == "waiting_for_message":
            ...
            if len(text) > MAX_URL_TEXT_LENGTH:
                error_text = (
                    ...
                    f"📏 *Максимальная длина:* {MAX_URL_TEXT_LENGTH} символов\n\n"
                    f"💡 *Сократите сообщение и попробуйте снова*"
                )
