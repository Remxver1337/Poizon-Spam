...
class SpamBot:
    ...
    async def handle_button(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            error_text = "❌ *Произошла ошибка при обработке запроса*\n\n💡 *Попробуйте еще раз или перезапустите бота командой /start*"
            try:
                await query.edit_message_text(error_text, parse_mode='Markdown')
            except:
                await context.bot.send_message(chat_id=query.from_user.id, text=error_text, parse_mode='Markdown')
    ...
    async def show_message_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not messages:
            no_messages_text = "📭 *У вас нет созданных сообщений*\n\n💡 *Создайте первое сообщение для работы*"
    ...
    async def generate_variations_with_timeout(self, text: str, count: int = 500) -> List[str]:
        ...
        try:
            variations = await asyncio.wait_for(
                asyncio.get_event_loop().run_in_executor(None, self._generate_variations_sync, text, count),
                timeout=5.0
            )
            return variations
        except asyncio.TimeoutError:
            logger.warning(f"Таймаут генерации вариаций для текста: {text[:50]}...")
            raise TimeoutError("Генерация вариаций заняла слишком много времени")

    def _generate_variations_sync(self, text: str, count: int = 500) -> List[str]:
        """Синхронная генерация вариаций"""
        variations = set()
        ...
        while len(variations) < max_possible_variations:
            ...
            for i, char in enumerate(variation):
                if char in REPLACEMENTS and random.random() < 0.6:
    ...
    async def show_chat_list(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if not chats:
            no_chats_text = "📭 *У вас нет добавленных чатов*\n\n💡 *Добавьте первый чат с пользователями*"
    ...
    async def show_spam_menu(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        try:
            ...
            menu_text = f"🚀 *Начать рассылку*\n\n📋 *Доступно чатов: {len(chats)}*\n\n💡 *Выберите чат для рассылки:*"
    ...
    async def show_users_for_spam(self, update: Update, context: ContextTypes.DEFAULT_TYPE, chat_id: int, page: int = 0):
        ...
        try:
            ...
            if chat_name == "Ошибка загрузки":
                await query.edit_message_text("❌ *Чат не найден*\n\n💡 *Вернитесь к списку чатов*", parse_mode='Markdown')
                return
            ...
            for user_id_db, username in users:
                if has_variations:
                    ...
                    if variation_text:
                        # Ссылка БЕЗ кодирования - используем текст как есть
                        spam_link = f"https://t.me/{username}?text={variation_text}"
                        ...
                        keyboard.append([InlineKeyboardButton(f"❌ {username} (нет вариаций)", callback_data="no_action")])
                else:
                    keyboard.append([InlineKeyboardButton(f"❌ {username}", callback_data="no_action")])
            ...
            error_text = "❌ *Ошибка при загрузке пользователей*\n\n💡 *Попробуйте еще раз или проверьте базу данных*"
            await query.edit_message_text(error_text, parse_mode='Markdown')
        ...
        try:
            if current_state == "waiting_for_message":
                ...
                try:
                    ...
                    if not variations:
                        await update.message.reply_text("❌ *Не удалось создать вариации*\n\n💡 *Попробуйте другое сообщение*", parse_mode='Markdown')
                        return
                    ...
                    self.delete_user_state(user_id)
                    await update.message.reply_text("❌ *Генерация заняла слишком много времени*\n\n💡 *Попробуйте более короткое сообщение*", parse_mode='Markdown')
                except Exception as e:
                    self.delete_user_state(user_id)
                    logger.error(f"Ошибка генерации вариаций: {e}")
                    await update.message.reply_text("❌ *Произошла ошибка при генерации вариаций*\n\n💡 *Попробуйте еще раз*", parse_mode='Markdown')
            
            elif current_state == "waiting_for_chat_name":
                if not text:
                    await update.message.reply_text("❌ *Название чата не может быть пустым*", parse_mode='Markdown')
                    return
                
                context.user_data['current_chat_name'] = text
                self.set_user_state(user_id, "waiting_for_users")
                
                users_text = f"🏷️ *Название чата сохранено:* {text}\n\n📝 *Отправьте список пользователей в столбик:*\n\n💡 *Каждый username с новой строки*"
                ...
                if cleaned_usernames:
                    ...
                    error_text = "❌ *Список пользователей пуст или содержит некорректные username*\n\n💡 *Отправьте список username'ов в столбик (минимум 5 символов)*"
                    await update.message.reply_text(error_text, parse_mode='Markdown')
                    
        except Exception as e:
            logger.error(f"Ошибка в handle_text_input: {e}")
            error_text = "❌ *Произошла ошибка при обработке данных*\n\n💡 *Попробуйте еще раз*"
            await update.message.reply_text(error_text, parse_mode='Markdown')
