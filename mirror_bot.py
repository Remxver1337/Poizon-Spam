...
class MirrorBot:
    ...
    def __init__(self, bot_token: str, creator_id: int, mirror_id: int, 
        ...
        logger.info(f"Зеркальный бот создан: mirror_id={mirror_id}, creator_id={creator_id}")
    ...
    async def handle_text_input(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if state == "waiting_for_message":
            ...
            variations = self.generate_variations(text, config.MAX_VARIATIONS_PER_MESSAGE)
    ...
    def generate_variations(self, text: str, count: int = 500) -> List[str]:
        ...
        while len(variations) < count:
            ...
            for i, char in enumerate(variation):
                if char in config.REPLACEMENTS and random.random() < 0.3:
                    variation[i] = config.REPLACEMENTS[char]
                    changes_made = True
    ...
    def run(self):
        """Запуск бота"""
        logger.info(f"Запуск зеркального бота {self.mirror_id}")
        
        if config.USE_WEBHOOK and self.host_domain:
            # Запуск с вебхуком
            webhook_url = f"https://{self.host_domain}:{self.webhook_port}/{self.token}"
            ...
            try:
                ...
                logger.info(f"Зеркало {self.mirror_id} запущено на вебхуке: {webhook_url}")
            except Exception as e:
                logger.error(f"Ошибка запуска вебхука: {e}")
                logger.info(f"Запускаем зеркало {self.mirror_id} в режиме polling")
                self.application.run_polling()
        else:
            # Запуск в режиме polling
            logger.info(f"Зеркало {self.mirror_id} запущено в режиме polling")
            self.application.run_polling()

# Класс SpamBot из вашего скрипта (для обратной совместимости)
class SpamBot:
    ...
    # ... (остальные методы SpamBot из вашего скрипта)
